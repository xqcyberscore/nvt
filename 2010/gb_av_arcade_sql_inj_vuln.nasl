...
# $Id: gb_av_arcade_sql_inj_vuln.nasl 5263 2017-02-10 13:45:51Z teissa $
#
...
if(!can_host_php(port:acPort)){
  exit(0);
}

foreach dir (make_list_unique("/avarcade", "/avarcade/upload/" , cgi_dirs(port:acPort)))
{

  if(dir == "/") dir = "";

  ## Send and Receive request
  rcvRes = http_get_cache(item: dir + "/index.php", port:acPort);

  ## Confirm application is AV Arcade
  if(">AV Arcade" >< rcvRes && ">AV Scripts</" >< rcvRes)
  {
    ## Try to get the Admin page to grep the version
    sndReq = http_get(item: dir + "/admin/stats.php", port:acPort);
