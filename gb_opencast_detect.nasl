...
# $Id: gb_opencast_detect.nasl 7941 2017-11-30 10:47:02Z jschulte $
#
# Opencast detection
#
# Authors:
...
if( description )
{
  ...
  script_version("$Revision: 7941 $");
  script_tag(name:"last_modification", value:"$Date: 2017-11-30 11:47:02 +0100 (Thu, 30 Nov 2017) $");
...
foreach dir ( make_list_unique( "/", "/admin-ng", cgi_dirs( port: port ) ) ) {
  foreach file ( make_list( "/", "/login.html" ) ) {
    ...
    if( match = eregmatch( pattern: "<title>Opencast[^<]{0,}", string: resp ) ) {
      ...
      build_report( app: "Opencast", ver: version, concluded: version_match[0], cpe: cpe, insloc: dir, port: port );
