...
# $Id: gb_joomla_classifiedsmanager_mult_vuln.nasl 3497 2016-06-13 12:28:47Z benallard $
#
...
if(description)
{
  ...
  - Input passed via the 'view' parameter to /classifieds script is not validated
  before returning it to users.
  - Input passed via the 'id' parameter to /classifieds/offerring-ads script is
  not properly sanitized before returning it to users.");
  ...
  script_summary("Check if Joomla J-ClassifiedsManager is vulnerable to SQL");
