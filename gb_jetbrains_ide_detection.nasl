...
# $Id: gb_jetbrains_ide_detection.nasl 7036 2017-09-01 04:41:05Z ckuersteiner $
#
...
if (description)
{
 ...
 script_version ("$Revision: 7036 $");
 script_tag(name: "last_modification", value: "$Date: 2017-09-01 06:41:05 +0200 (Fri, 01 Sep 2017) $");
 ...
 script_tag(name: "summary" , value: "Detection of jetBrains IDE. 

Tries to detect jetBrains IDE via http Get request.");
...
req = http_get_req( port: port, url: url, data: data, add_headers: make_array( 'Content-Type', 'appllication/xml',
                                                                               'Connection', 'close' ) );
res = http_keepalive_send_recv( port: port, data: req );

if ( "server: PyCharm" >!< res && "server: WebStorm" >!< res && "server: CLion" >!< res &&
     "server: DataGrip" >!< res && "server: IntelliJ IDEA" >!< res && "server: JetBrains MPS" >!< res &&
     "server: PyCharm Edu" >!< res && "server: jetBrains Rider" >!< res && "server: RubyMine" >!< res )
  exit ( 0 );
