...
# $Id: gb_lantronix_device_detect_snmp.nasl 7929 2017-11-29 09:59:29Z cfischer $
#
...
if( sysdesc =~ "^Lantronix" ) {
  ...
  version = "unknown";
  type    = "unknown";
  vers_nd_type = eregmatch( pattern:"^Lantronix ([A-Z0-9]+) .* V([0-9.]+)", string:sysdesc );
  if( vers_nd_type[1] ) type    = vers_nd_type[1];
  if( vers_nd_type[2] ) version = vers_nd_type[2];
