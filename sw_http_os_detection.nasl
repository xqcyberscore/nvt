...
function check_php_banner( port, host ) {
  ...
  foreach checkFile( checkFiles ) {
    ...
    if( egrep( pattern:"^X-Powered-By: PHP/[0-9.]+(-[0-9.]+)?$", string:phpBanner ) ) {
      phpBanner = NULL;
      continue;
    }
...
function check_x_powered_by_banner( port, banner ) {
  ...
  if( banner && banner = egrep( pattern:"^X-Powered-By:.*$", string:banner, icase:TRUE ) ) {
    ...
    if( " PHP" >< banner || egrep( pattern:"^X-Powered-By: PHP/[0-9.]+(-[0-9]+)?$", string:banner, icase:TRUE ) ) return;
    ...
    if( egrep( pattern:"^X-Powered-By: Servlet/([0-9.]+)$", string:banner, icase:TRUE ) ) return;
