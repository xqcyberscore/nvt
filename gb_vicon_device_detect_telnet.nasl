...
if(description)
{
  ...
  script_name("Vicon Devices Detection (Telnet)");
  ...
  script_tag(name:"summary", value:"This script performs Telnet based detection of Vicon Devices.");
...
if( egrep( string:banner, pattern:"^IQinVision .* Version ", icase:FALSE ) ||
  ...
  set_kb_item( name:"vicon_device/telnet/detected", value:TRUE );
  set_kb_item( name:"vicon_device/telnet/port", value:port );
}
  version = "unknown";
  ...
  if( vers[2] ) {
    version = vers[2];
    set_kb_item( name:"vicon_device/telnet/" + port + "/concluded", value:vers[0] );
  } else {
    set_kb_item( name:"vicon_device/telnet/" + port + "/concluded", value:bin2string( ddata:banner, noprint_replacement:'' ) );
  }
  set_kb_item( name:"vicon_device/telnet/" + port + "/version", value:version );
  ...
  if ( type == "unknown" ) {
    ...
    if( soc ) {
      ...
      if ( "prompt for assistance" >< recv1 && "Username>" >< recv1 ) {
        ...
        if ( recv2 =~ "Local_.+>" ) {
          access = TRUE;
          set_kb_item(name:"vicon_device/telnet/" + port + "/access", value:TRUE );      
        }
      }
  ...
  set_kb_item( name:"vicon_device/telnet/" + port + "/type", value:type );
  ...
  if( mac = eregmatch( pattern:"MAC address ([0-9a-fA-F]{12})", string:bin2string( ddata:banner, noprint_replacement:'' ) ) ) {
    ...
    register_host_detail( name:"MAC", value:full_mac, desc:"Get the MAC Address via IQinVision Telnet banner" );
