...
foreach dir( make_list_unique( "/" , "/forum" , "/forums" , "/xenforo" , "/xf" , "/board" , "/boards" , cgi_dirs(port:port) ) ) {
  ...
  if( ">Forum software by XenForo" >< rcvRes || "jQuery.extend(true, XenForo," >< rcvRes ) {
    installed = 1;
  } else {
  ...
  if( installed ) {
    ...
    set_kb_item( name:"xenforo/installed",value:TRUE );
    tmp_version = ver + " under " + install;
    set_kb_item( name:"www/" + port + "/xenforo", value:tmp_version );
   
    ## Build CPE
    ## CPE not reqistered yet
    cpe = build_cpe( value:ver, exp:"^([0-9.]+)", base:"cpe:/a:xenforo:xenforo:" );
    if( isnull( cpe ) )
