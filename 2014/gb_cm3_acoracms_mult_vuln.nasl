...
# $Id: gb_cm3_acoracms_mult_vuln.nasl 3517 2016-06-14 12:46:45Z benallard $
#
...
if(description)
{
  ...
  script_summary("Check if CM3 AcoraCMS is vulnerable to cross site scripting");
...
foreach dir (make_list_unique("/", "/AcoraCMS", "/cms", cgi_dirs(port:cmsPort)))
{
  ...
  sndReq = http_get(item:string(dir, "/Admin/login/default.asp"), port:cmsPort);
  ...
  if("Welcome to the Acora CMS web-based administration" >< rcvRes)
  {
    ...
    url = dir + '/Admin/login/default.asp?username="</div><script>alert(docu'+
                'ment.cookie)</script>';
