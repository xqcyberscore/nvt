...
tag_insight = "Open Source SIEM (OSSIM) is prone to a directory-traversal
vulnerability because it fails to sufficiently sanitize user-supplied input.";

tag_impact = "Exploiting this issue can allow an attacker to gain access to
arbitrary system files. Information harvested may aid in launching
further attacks.";

tag_affected = "All AlienVault Versions prior to v4.3.3.1";

tag_summary = "AlienVault Open Source SIEM (OSSIM) 'timestamp' Parameter Directory
...
if( buf =~ "HTTP/1.. 200" && buf =~ "Content-Disposition: attachment; filename=.*\.zip" )
{
