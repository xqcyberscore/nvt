...
if( ! smbPort ) smbPort = 139;
if( ! get_port_state( smbPort ) ) exit( 0 );

smbName = kb_smb_name();
if( ! smbName ) smbName = "*SMBSERVER";
...
foreach credential( credentials ) {
  ...
  if( tolower( password ) == "none" ) password = "";

  login_defined = remote_login( smbLogin:smbLogin, passwd:password, smbDomain:"", smbName:smbName, smbPort:smbPort );
  if( login_defined ) {
    report = string( "It was possible to login with the following credentials via the SMB protocol. ",
                     "<User>:<Password>\n\n", smbLogin, ":", password, "\n" );
