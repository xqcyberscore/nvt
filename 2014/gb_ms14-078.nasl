...
# $Id: gb_ms14-078.nasl 3008 2016-04-07 13:17:01Z antu123 $
#
...
if(description)
{
  ...
  script_summary("Check for the vulnerable 'Imjputyc.dll' file version");
...
{
  ## Confirm IME Japanese is present
  key = "SOFTWARE\Microsoft\IMEJP\8.1";
  if(!registry_key_exists(key:key)){
    exit(0);
  }

  dllVer = fetch_file_version(sysPath, file_name:"IME\IMJP8_1\Imjputyc.dll");
  if(!dllVer){
    exit(0);
  }
...
dllVer = fetch_file_version(sysPath, file_name:"system32\IME\IMEJP10\Imjputyc.dll");
if(!dllVer){
  exit(0);
}
...
{
  ## Check for Imjputyc.dll version
