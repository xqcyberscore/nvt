...
# $Id: mssqlserver_detect.nasl 4034 2016-09-12 12:12:26Z cfi $
#
...
if(description)
{
  ...
  script_summary("Microsoft's SQL TCP/IP listener is running");
...
function check( port )
{
  ...
  req = raw_string( 0x12, 0x01 ) + mkword( len + 8 ) + 
        raw_string( 0x00, 0x00, 0x00, 0x00 ) + payload;
  ...
  if( blen < 6 || ( pos + 6 ) > strlen( buf ) ) return FALSE;
  ...
  log_message( data: build_detection_report( app:"Microsoft SQL Server",
                                             version:version,
                                             install:port + '/tcp',
                                             cpe:cpe,
