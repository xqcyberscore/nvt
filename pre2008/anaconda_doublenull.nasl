...
# $Id: anaconda_doublenull.nasl 2846 2016-03-14 09:13:09Z cfi $
...
if(description)
{
 ...
 script_summary("Anaconda Foundation Directory Double NULL Encoded Remote File Retrieval");
...
foreach dir (make_list_unique("/cgi-local", cgi_dirs(port:port)))
{
  ...
  foreach file(keys(files)) {
    ...
    if(http_vuln_check(port:port, url:item, pattern:file,
                       check_header:TRUE)) {
