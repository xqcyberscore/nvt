...
# $Id: gb_dell_sonicwall_email_security_detect.nasl 2836 2016-03-11 09:07:07Z benallard $
#
...
to extract the version number from the reply.";
 ...
 script_summary("Checks for the presence of Dell SonicWall EMail Security");
...
{  
  ...
  {  
    ...
    if( smtp_banner =~ "220.*SonicWALL" )
    {
      ...
      if( ! isnull( version[1] ) )
        _report( version:version[1], port:smtp_port );
      else 
        _report( version:'unknown', port:smtp_port );
    }  
  }
}  

http_ports = get_kb_list( "Services/www" );
if(! http_ports ) exit(0);
  ...
  if( "<title>Login</title>" >< buf && ">Email Security" >< buf && "Dell" >< buf )
  {
    ...
    if( ! isnull( version[1] ) )
    {
      ...
      if( ! isnull( version[1] ) )
        ...
        _report( version:'unknown', port:http_port );
    }
  }  
}  
