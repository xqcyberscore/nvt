...
# $Id: gb_zencart_code_exec_vuln.nasl 4970 2017-01-09 15:00:59Z teissa $
#
...
if(description)
{
  ...
  - Access to admin/record_company.php is not restricted and can be exploited
    ...
    the file in images/.");
  script_tag(name : "solution" , value : "Apply security patch from below link,
...
foreach dir (make_list_unique("/", "/zencart", "/cart", cgi_dirs(port:zencartPort)))
{
  ...
  rcvRes = http_keepalive_send_recv(data:sndReq, port:zencartPort);
