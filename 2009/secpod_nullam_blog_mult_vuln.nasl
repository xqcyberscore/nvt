...
# $Id: secpod_nullam_blog_mult_vuln.nasl 7577 2017-10-26 10:41:56Z cfischer $
#
...
if(description)
{
  ...
  - Input passed to the 'e' parameter in index.php is not properly sanitised before being returned to the user.
    ...
    of an affected site.");
  script_tag(name:"solution", value:"Upgrade to Nullam Blog version 0.1.3
...
foreach dir( make_list_unique("/", "/nullam", "/blog", cgi_dirs( port:port ) ) ) {
  ...
  if( rcvRes1 =~ "HTTP/1\.. 200" && "<title>Nullam</title>" >< rcvRes1 ) {
    ...
    url = dir + "/index.php?p=error&e=<script>alert" + "('OpenVAS-SQL-Injection-Test');</script>";
    sndReq2 = http_get( item:url, port:port );
    rcvRes2 = http_keepalive_send_recv( port:port, data:sndReq2 );

    if( rcvRes2 =~ "HTTP/1\.. 200" && "<script>alert('OpenVAS-SQL-Injection-Test');</script>" >< rcvRes2 ) {
      report = report_vuln_url( port:port, url:url );
      security_message( port:port, data:report );
      exit( 0 );
    }
