    ...
    "lucene_version" >< buf && ( "elasticsearch" >< buf || "You Know, for Search" >< buf ) )
{
  ...
  set_kb_item(name:string("www/", port, "/elastisearch"), value: vers);
  set_kb_item(name:"elastisearch/installed",value:TRUE);

  register_product(cpe:cpe, location:"/", port:port);

  log_message(data: build_detection_report( app:"Elastisearch",
                                            version:vers,
                                            install:'/',
                                            cpe:cpe,
                                            concluded: vers),
