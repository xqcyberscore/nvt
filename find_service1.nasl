...
if(description)
{
  ...
  script_name("Identify unknown services with 'GET'");
...
{
 register_service(port: port, proto: 'mysql', message:'A MySQL server seems to be running on this port but it rejects connection from the openvas scanner.');	# or wrapped?
 log_message(port: port, data:"A MySQL server seems to be running on this port but it
...
{
 register_service(port: port, proto: 'mysql', message:'A MySQL server seems to be running on this port but the OpenVAS scanner IP has been blacklisted.\nRun "mysqladmin flush-hosts" if you want complete tests.');
