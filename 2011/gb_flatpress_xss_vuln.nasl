...
# $Id: gb_flatpress_xss_vuln.nasl 3507 2016-06-14 04:32:30Z ckuerste $
#
...
if(description)
{
  ...
  script_tag(name : "insight" , value : "The flaw is due to input passed to 'name', 'email' and 'url' POST
  parameters in index.php are not properly sanitised before returning to the user.");
  script_tag(name : "solution" , value : "Upgrade FlatPress 0.1010.2 or later,
  For updates refer to http://flatpress.org/home/");
  script_tag(name : "summary" , value : "This host is running FlatPress and is prone to cross site
...
foreach path (make_list_unique("/blog","/FlatPress", cgi_dirs(port:port)))
{
