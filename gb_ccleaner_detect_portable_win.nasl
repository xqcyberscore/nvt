...
# $Id: gb_ccleaner_detect_portable_win.nasl 11627 2018-09-26 15:05:38Z cfischer $
#
...
fileList = wmi_file_file_search( handle:handle, fileName:"ccleaner", fileExtn:"exe", includeHeader:FALSE );
if( ! fileList ) {
...
foreach filePath( fileList ) {
  ...
  foreach vers( keys( versList ) ) {
    ...
    if( versList[vers] && version = eregmatch( string:versList[vers], pattern:"^([0-9]+\.[0-9]+\.[0-9]+)" ) ) {
      ...
      register_and_report_cpe( app:"CCleaner Portable", ver:version[1], concluded:versList[vers], base:cpe, expr:"^([0-9.]+)", insloc:location );
    }
  }
}
