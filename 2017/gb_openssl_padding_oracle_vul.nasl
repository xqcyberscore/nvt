...
# $Id: gb_openssl_padding_oracle_vul.nasl 11874 2018-10-12 11:28:04Z mmartin $
#
  ...
  )
{
  ...
  if( ! sslversion = get_supported_tls_version( port:port, min:TLS_10 ) ) exit( 0 );
  ...
  hello = ssl_hello(version:sslversion, ciphers:selectedCipherSuite, random:clientRandom, handshake_version:protocolversion);
...
##Sending ClientKeyExchange message
  ...
  for (i = offset; i< readMacsize; i=i+1)
  {
  ...
  for (i = offset; i< offset+writeMacsize; i=i+1)
  {
  ...
  for (i = offset; i< offset+KeySize; i=i+1)
  {
