...
##Get build Version
build = get_kb_item( "Hp/data_protector/build" );

##Check for version less than 9.x before 9.06
if( vers =~ "^(9\.)") {
  if( version_is_less( version:vers, test_version:"9.06" ) ) {
    fix = '9.06';
    VULN = TRUE;
  }
...
##Check for version less than 8.x before 8.15
  ...
  if( version_is_less( version:vers, test_version:"8.15" ) ) {
    fix = '8.15';
    VULN = TRUE;
  }
...
} else if( build && vers =~ "^(7\.03)" ) {
  if( version_is_less( version:build, test_version:"108" ) ) {
    report = report_fixed_ver( installed_version:vers + "_" + build, fixed_version:"7.03_108");
    security_message( data:report, port:port );
    exit(0);
  }

####Check for version less than 7.03
} else if( version_is_less( version:vers, test_version:"7.03" ) ) {
  fix = '7.03_108';
  VULN = TRUE;
}
