...
if(description)
{
  ...
  or extract files from a zip archive.  Zip archives are commonly found on
  MS-DOS systems.  The zip utility, included in the zip package, creates zip
  archives.  Zip and unzip are both compatible with archives created by
  PKWARE(R)&#39 s PKZIP for MS-DOS, but the programs&#39  options and default
  behaviors do differ in some respects.
