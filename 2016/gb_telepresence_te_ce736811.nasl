...
tag_impact = "A vulnerability in the XML application programming interface (API) of 
Cisco TelePresence Codec (TC) and Collaboration Endpoint (CE) Software could allow 
an unauthenticated, remote attacker to bypass authentication and access a targeted system through the API..";

tag_insight = "The vulnerability is due to improper implementation of authentication mechanisms for the XML
 API of the affected software. An attacker could exploit this vulnerability by sending a crafted HTTP request
 to the XML API. A successful exploit could allow the attacker to perform unauthorized configuration changes 
or issue control commands to the affected system by using the API.";

tag_affected = "This vulnerability affects Cisco TelePresence Software releases TC 7.2.0, TC 7.2.1, TC 7.3.0, 
...
";
tag_summary = "Cisco TelePresence Codec (TC) and Collaboration Endpoint (CE) Softwares are vulnerable to a 
