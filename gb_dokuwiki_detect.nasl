...
# $Id: gb_dokuwiki_detect.nasl 5145 2017-01-31 11:07:58Z cfi $
#
...
foreach dir( make_list_unique( "/", "/wiki", "/dokuwiki", cgi_dirs( port:port ) ) ) {
  ...
  # Check if the install is missing a patch. The output of this notify area
  # is currently available at http://update.dokuwiki.org/check/
  ...
  if( ! vers[2] ) {
    # Try to get version from an unprotected VERSION file
    url = dir + "/VERSION";
