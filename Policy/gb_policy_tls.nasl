...
if( ! minimum_TLS ) exit( 0 );

set_kb_item( name:'tls_policy/minimum_TLS', value:minimum_TLS );

supported_versions = get_kb_list("tls_version_get/" + port + "/version");
if( ! supported_versions ) exit( 0 );

supported_versions = sort( supported_versions );

ssl["SSLv2"]   = raw_string(0x02,0x00);
ssl["SSLv3"]   = raw_string(0x03,0x00);
ssl["TLSv1.0"] = raw_string(0x03,0x01);
ssl["TLSv1.1"] = raw_string(0x03,0x02);
ssl["TLSv1.2"] = raw_string(0x03,0x03);
