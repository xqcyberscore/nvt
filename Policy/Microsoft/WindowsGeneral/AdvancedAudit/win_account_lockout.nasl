...
# $Id: win_account_lockout.nasl 11068 2018-08-21 11:51:41Z emoss $
#
# Check value for Audit Account Lockout
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.109584");
  ...
  script_name('Microsoft Windows: Audit Account Lockout');
...
'Audit Account Lockout' on Windows hosts (at least Windows 7).

The policy setting enables the audit security events that are generated by a failed attempt to log
on to an account that is locked out. An audit event is generated when an account cannot log on to a
computer because the account is locked out.");
  exit(0);
}
...
key = "WMI/AdvancedPolicy/AccountLockout";
