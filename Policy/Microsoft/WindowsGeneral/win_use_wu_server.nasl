...
# $Id: win_use_wu_server.nasl 9516 2018-04-18 08:02:49Z emoss $
#
...
value = registry_get_dword(key:key, item:'UseWUServer', type:'HKLM');
if( value == ''){
  policy_logging(text:'Unable to detect registry value "HKLM\\Software\\Policies\\Microsoft\\Windows\\WindowsUpdate\\AU!UseWUServer".');
  set_kb_item(name:'Policy/MS/Windows/AU/UseWUServer', value:'none');
}else{
  policy_logging(text:'Registry value "HKLM\\Software\\Policies\\Microsoft\\Windows\\WindowsUpdate\\AU!UseWUServer" is set to: ' + value);
  set_kb_item(name:'Policy/MS/Windows/AU/UseWUServer', value:value);
}
