...
# $Id: office_no_ui_extending_excel.nasl 12129 2018-10-26 13:57:15Z cfischer $
#
# Check value for Disable UI extending from documents and templates (Excel)
#
# Authors:
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.109660");
  ...
  script_name('Microsoft Excel: Disable UI Extending from Documents and Templates');
...
from documents and templates' for Microsoft Excel 2013 (at least) on Windows hosts.

The policy setting controls whether Microsoft Excel loads any custom user interface (UI) code
included with a document or template. Excel allows developers to extend the UI with customization
...
title = 'Disable UI Extending from Documents and Templates (Excel)';
fixtext = 'Set following UI path accordingly:
User Configuration/Administrative Templates/Microsoft Office ' + office_year + '/Global Options/Customize/' + title;
type = 'HKU';
key_root = 'Software\\Policies\\Microsoft\\Office\\' + major_version + '\\common\\toolbars\\excel';
