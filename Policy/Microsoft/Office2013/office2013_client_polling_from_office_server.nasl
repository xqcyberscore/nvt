...
# $Id: office2013_client_polling_from_office_server.nasl 11843 2018-10-11 14:33:21Z emoss $
#
# Check value for Disable the Office client from polling the SharePoint
...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.109060");
  ...
  script_name('Microsoft Office: Disable the Office client from polling the SharePoint Server for published links');
  ...
  script_tag(name:"summary", value:"This test checks the setting for policy 'Disable the Office
client from polling the SharePoint Server for published links' for Microsoft Office 2013 (at least)
on Windows hosts.

The setting controls whether Office applications can poll Office servers to retrieve lists of
published links.");
  exit(0);
}
...
title = 'Disable the Office client from polling the SharePoint Server for published links';
fixtext = 'Set following UI path accordingly:
User Configuration/Administrative Templates/Microsoft Office ' + office_year + '/Server Settings/' + title;
type = 'HKU';
key_root = 'Software\\Policies\\Microsoft\\Office\\' + major_version + '\\common\\portal';
