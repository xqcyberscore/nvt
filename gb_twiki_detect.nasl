...
foreach dir( dirs ) {
  ...
  if( rcvRes =~ "HTTP/1.. 200" && ( egrep( pattern:"[p|P]owered by TWiki", string:rcvRes ) || "This site is powered by the TWiki collaboration platform" >< rcvRes ) ) {
    ...
    if( ver[1] ) {
      ...
      ver = eregmatch( pattern:"This site is running TWiki version <strong>([a-zA-Z0-9 ]+)</strong>", string:rcvRes ); # nb: Old versions of Twiki before 4.0 had the release date as its version number.
      if( ver[1] ) {
