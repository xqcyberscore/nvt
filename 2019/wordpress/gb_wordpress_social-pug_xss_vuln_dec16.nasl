...
res = http_get_cache(port: port, item: dir + "/wp-content/plugins/social-pug/readme.txt");

if("Social Pug" >< res && "Changelog" >< res) {

  vers = eregmatch(pattern: "Stable tag: ([0-9.]+)", string: res);

  if(!isnull(vers[1]) && version_is_less(version: vers[1], test_version: "1.2.6")) {
    report = report_fixed_ver(installed_version: vers[1], fixed_version: "1.2.6");
