...
# $Id: secpod_sun_java_dir_server_detect_win.nasl 6065 2017-05-04 09:03:08Z teissa $
#
...
if(registry_key_exists(key:key1))
{
  foreach item (registry_enum_keys(key:key1))
  {
    ...
    if(ver[1] != NULL)
    {
      ...
      ## Build cpe
      cpe = build_cpe(value:ver[1], exp:"^([0-9.]+)", base:"cpe:/a:sun:java_system_directory_server:");
      if(isnull(cpe))
  ...
  if("Directory Server" >< appregCheck)
  {
    ...
    if("Directory Server" >< infContent)
    {
      ...
      if(appVer[1] != NULL)
      {
        ...
        ## Build cpe
        cpe = build_cpe(value:appVer[1], exp:"^([0-9.]+)", base:"cpe:/a:sun:java_system_directory_server:");
        if(isnull(cpe))
          cpe = 'cpe:/a:sun:java_system_directory_server';
