...
pngName = find_file(file_name:"libpng-config", file_path:"/", useregex:TRUE, regexpar:"$", sock:sock);
if(!pngName) {
...
foreach executableFile(pngName) {
  ...
  pngVer = get_bin_version(full_prog_name:executableFile, version_argv:"--version", ver_pattern:"[0-9.]{3,}", sock:sock);
  if(!isnull(pngVer[0])) {
