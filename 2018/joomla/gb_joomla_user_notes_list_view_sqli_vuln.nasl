...
if(description)
{
  ...
  script_tag(name:"summary", value:"This host is installed with Joomla and is
  prone to sql injection vulnerability.");

  script_tag(name:"vuldetect", value:"Get the installed version with the help
  of the detect NVT and check if the version is vulnerable or not.");

  script_tag(name:"insight", value:"The flaw exists due to Joomla's lack of type
  casting of a variable in a SQL statement in 'User Notes list view'.");

  script_tag(name:"impact", value:"Successful exploitation will allow an attackers
  to conduct SQL injection in the user notes list view.
...
{
 report = report_fixed_ver(installed_version:vers, fixed_versio:"3.8.6", install_path:path);
